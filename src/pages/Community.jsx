...
export default function Community() {
  ...
  const handleAddComment = async (id) => {
    if (!commentInputs[id]) { toast.info("Write a comment before posting."); return; }
  ...
  return (
    <div className="community-page">
      ...
      {showForm && (
        <SpotlightCard className="create-post-form" spotlightColor="rgba(124, 58, 237, 0.2)">
          ...
          <textarea name="content" placeholder="Write something..." value={newPost.content} onChange={handleInputChange} required />
        ...
        {posts.map((post) => (
          <SpotlightCard key={post.id} className="post-card">
            ...
            <div className="actions">
              ...
              <button className="btn-action" onClick={() => setCommentInputs({ ...commentInputs, [post.id]: commentInputs[post.id] ?? "" })}>
                💬 Comment
              </button>
            ...
            {post.comments.length > 0 && (
              <div className="comments-list">
                {post.comments.map((c, i) => <p key={i} className="comment">{c}</p>)}
              </div>
            )}
          </SpotlightCard>
        ))}
